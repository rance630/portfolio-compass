...
def _secret_value(*names: str) -> str | None:
    """從 Streamlit secrets 讀取設定，支援多種常見命名。"""
    for name in names:
        try:
...
def sheets_configured() -> bool:
    """判斷目前是否已設定 Google Sheets 作為雲端儲存。"""
    if gspread is None or Credentials is None:
        return False
    sheet_id = _secret_value("GOOGLE_SHEET_ID", "SHEET_ID", "google_sheet_id", "sheet_id")
    try:
        _ = st.secrets["gcp_service_account"]
        has_account = True
    except Exception:
        has_account = False
    return bool(sheet_id and has_account)


@st.cache_resource(show_spinner=False)
def get_google_sheet():
    """連線到 Google Sheet。沒有設定 secrets 時不會呼叫。"""
...
def get_or_create_worksheet(title: str, headers: list[str], rows: int = 200):
    ...
    try:
        ...
        worksheet = sheet.add_worksheet(title=title, rows=rows, cols=max(len(headers), 2))
...
def worksheet_records(title: str, headers: list[str]) -> pd.DataFrame:
...
def write_worksheet(title: str, headers: list[str], frame: pd.DataFrame) -> None:
    worksheet = get_or_create_worksheet(title, headers)
    if worksheet is None:
        return
    safe = frame.copy()
    for col in headers:
        if col not in safe.columns:
            safe[col] = ""
    safe = safe[headers].fillna("")
    values = [headers] + safe.astype(str).values.tolist()
    worksheet.clear()
...
def load_portfolio() -> pd.DataFrame:
    if sheets_configured():
        try:
            df = worksheet_records(HOLDINGS_SHEET_NAME, DEFAULT_COLUMNS)
        except Exception as exc:
            st.warning(f"Google Sheets 讀取失敗，暫時改用本機 CSV：{exc}")
...
def save_portfolio(df: pd.DataFrame) -> None:
    ...
    clean["買入日期"] = pd.to_datetime(clean["買入日期"], errors="coerce").dt.date.astype("string").fillna("")
    clean["備註"] = clean["備註"].fillna("").astype(str)
    clean = clean[(clean["股票代號"] != "") & (clean["股數"] > 0)]

    if sheets_configured():
        try:
            write_worksheet(HOLDINGS_SHEET_NAME, DEFAULT_COLUMNS, clean)
            return
        except Exception as exc:
            st.error(f"Google Sheets 儲存失敗：{exc}")
            st.stop()
...
def load_cash_twd() -> float:
    if sheets_configured():
        try:
            settings = worksheet_records(SETTINGS_SHEET_NAME, SETTINGS_COLUMNS)
            if not settings.empty:
                match = settings[settings["key"].astype(str) == "cash_twd"]
                if not match.empty:
                    return max(float(pd.to_numeric(match.iloc[0]["value"], errors="coerce") or 0), 0.0)
        except Exception as exc:
            st.warning(f"Google Sheets 現金水位讀取失敗，暫時改用本機 CSV：{exc}")
...
def save_cash_twd(value: float) -> None:
    ...
    if sheets_configured():
        try:
            write_worksheet(SETTINGS_SHEET_NAME, SETTINGS_COLUMNS, pd.DataFrame([{"key": "cash_twd", "value": value}]))
            return
        except Exception as exc:
            st.error(f"Google Sheets 現金水位儲存失敗：{exc}")
            st.stop()
    pd.DataFrame([{"現金水位（台幣）": value}]).to_csv(CASH_PATH, index=False, encoding="utf-8-sig")
...
with st.sidebar:
    ...
    st.markdown(f"<div class='small-muted'>目前儲存方式：{storage_label()}</div>", unsafe_allow_html=True)
...
st.markdown(
    ...
    <div class="pill-row">
        ...
        <div class="pill">儲存方式：{storage_label()}</div>
